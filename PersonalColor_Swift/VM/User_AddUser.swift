...
class User_ImageUpload{
    ...
    func imageUploadUpdate(DocumentId: String, image: Data, upassword: String,uname:String){
        // 오늘 날짜시간 file명으로 바꾸기
        let date = DateFormatter()
        date.locale = Locale(identifier: "ko_kr")
        date.dateFormat = "yyyy-MM-dd HH:mm:ss"
        ...
        _ = riversRef.putData(image, metadata: nil) {(metadata,error) in
            ...
            riversRef.downloadURL {(url, error) in
                ...
                updateDB.updateItems(DocumentId: DocumentId, image: image, upassword: upassword, uname: uname)
            }
